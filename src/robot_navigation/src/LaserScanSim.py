...
LASERSCAN_UPDATE_MIN = 0.2
LASERSCAN_UPDATE_MAX = 2.0
...
class LaserScanSim(LaserScan):
    def __init__(self):
        ...
        self._last_pose = Pose() # 上一次更新LaserScan的位置
        self._pose = Pose() # 最新一次更新pose的值
        ...
        self._last_time = rospy.Time.now() # 上一次更新LaserScan的時間
        self._update_time = self._last_time # 最新一次更新pose的時間
    ...
    def updateLaserScan(self,msg):# Update LasrScan when msg from /scan receive, only update rnage which is larger than min_range ( 0.12 )
        ...
        #     (r, p, _last_direction_tmp) = euler_from_quaternion([ self._pose.orientation.x,  self._pose.orientation.y,  self._pose.orientation.z, self._last_pose.orientation.w])
        #     last_update_direction = (_last_direction_tmp + 2*pi)%(2*pi)#range of _last_direction_tmp is -pi ~ pi, update them to 0 ~ 2pi

        #     if(self._last_twist.angular.z == 0):
        #         new_pose.position.x = self._pose.position.x + self._last_twist.linear.x * sin( last_update_direction ) * delta_LaserScanUpdate_now
        #         new_pose.position.y = self._pose.position.y + self._last_twist.linear.x * cos( last_update_direction ) * delta_LaserScanUpdate_now 
        #         new_pose.position.z = self._pose.position.z 
        #     else:
        ...
        #         new_pose.position.x = self._pose.position.x + linear_div_angular_velocity * sin( last_update_direction + angular_velocity_mul_time )
        #         new_pose.position.y = self._pose.position.y - linear_div_angular_velocity * cos( last_update_direction + angular_velocity_mul_time ) + linear_div_angular_velocity
        #         new_pose.position.z = self._pose.position.z 
